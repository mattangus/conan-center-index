...
class EmSDKConan(ConanFile):
    name = "emsdk"
    description = (
        "Emscripten SDK. Emscripten is an Open Source LLVM to JavaScript compiler"
    )
    ...
    def _tools_for_version(self):
        ...
        with open(
            os.path.join(self.source_folder, "emscripten-releases-tags.json"), "r"
        ) as f:
            data = json.load(f)
        ...
        with open(os.path.join(self.source_folder, "emsdk_manifest.json"), "r") as f:
            ...
            if self.settings.os == "Windows":
                python = next(
                    (
                        it
                        for it in tools
                        if (it["id"] == "python" and not it.get("is_old", False))
                    ),
                    None,
                )
            ...
            node = next(
                (
                    it
                    for it in tools
                    if (it["id"] == "node" and not it.get("is_old", False))
                ),
                None,
            )
